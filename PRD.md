...
- **현재 버전**: v1.7.0 (Data & Category Expansion)
- **최종 업데이트**: 2026-03-24
- **상태**: 최신 데이터셋 동기화 완료
...
- **콘텐츠 강화 (v1.7)**: '대시보드' 프로젝트를 포함한 총 25개의 프로젝트 기본 탑재.
- **완벽한 한글화**: 모든 인터페이스 한국어 대응.

### 2.3 데이터 영속성 (Data Persistence)
- **LocalStorage**: v1.7 키를 통해 최신 초기 데이터를 사용자 브라우저에 즉시 동기화.
...
## 5. 업데이트 기록
- **v1.6**: 24개의 대규모 프로젝트 데이터셋 동기화
- **v1.7**: 25번째 프로젝트(소방서 화재진압현황) 및 '대시보드' 카테고리 추가
